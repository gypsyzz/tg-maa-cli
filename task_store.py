...
def load_task_json(name: str, ) -> tuple[Path, dict]:
    ...
    if not isinstance(
        ...
        raise ValueError(
            f"{path} must contain a 'tasks' list."
        )
...
def short_task_sequence(data: dict, ) -> str:
    ...
    for index, task in enumerate(
        ...
        if kind == "Fight":
            lines.append(
                f"{index}. Fight - {fight_stage(task)}"
            )
        else:
            lines.append(
                f"{index}. {kind}"
...
def fight_sequence(data: dict, ) -> str:
    ...
    for index, task in enumerate(
        ...
        if task_type(task) == "Fight":
            lines.append(
                f"{index}. {fight_stage(task)}"
            )

    return (
        "\n".join(lines)
        if lines
        else "No Fight tasks configured."
    )
...
def parse_fight_add_args(values: list[str], ) -> tuple[str, int | None]:
    if not values:
        raise ValueError(
            "Usage: /fight add STAGE [POSITION]"
        )
    ...
    if (
        ...
        raise ValueError(
            "Fight position cannot be negative."
        )
...
def add_fight_task(data: dict, stage: str, position: int | None, ) -> tuple[int, int]:
    ...
    if not fights:
        raise ValueError(
            "No existing Fight task is available "
    ...
    for _, task in fights:
        if fight_stage(task) == stage:
            raise ValueError(
                f"Fight stage {stage!r} already exists."
            )
    ...
    if position > fight_count:
        raise ValueError(
            f"Fight position {position} is out of range. "
            f"Valid positions are 0 to {fight_count}."
        )
    ...
    if not isinstance(params, dict):
        raise ValueError(
            "The Fight template has no valid params object."
        )
...
def remove_fight_task(data: dict, task_index: int, ) -> str:
    if task_index < 1:
        raise ValueError(
            "Task index must be 1 or greater."
        )
    ...
    if kind != "Fight":
        raise ValueError(
            f"Task {task_index} is {kind}, not Fight."
        )
