...
async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE, ) -> None:
    ...
    if action == "remove":
        ...
        try:
            current = get_profile(name).schedule
            remove = set(
                normalize_times(values)
            )
            times = [
                value
                for value in current.times
...
async def task_command(update: Update, context: ContextTypes.DEFAULT_TYPE, ) -> None:
    ...
    mode = (
        "short"
        if not context.args
        else context.args[0].lower()
    )

    if mode == "short":
        await reply_preformatted(
            update,
            name,
            text_for(
                lang,
...
async def fight_command(update: Update, context: ContextTypes.DEFAULT_TYPE, ) -> None:
    ...
    if action == "add":
        try:
            stage, position = parse_fight_add_args(
                values
            )
            overall_index, fight_position = (
                add_fight_task(
                    data,
                    stage,
                    position,
...
async def log_command(update: Update, context: ContextTypes.DEFAULT_TYPE, ) -> None:
    ...
    if requested not in {"OFF", "ON", "FULL"}:
        await update.effective_message.reply_text(
            "❌ "
            + (
                "日志模式必须是 OFF、ON 或 FULL。"
                if lang == "zh"
                else "Log mode must be OFF, ON, or FULL."
            )
        )
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, ) -> None:
    ...
    name = current_name(context)
    lang = current_lang(name)
    action = query.data.removeprefix(
        "maa:"
    )
